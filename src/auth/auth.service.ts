...
export class AuthService {
  ...
  async register(registerDto: RegisterDto) {
    try {
      ...
      const createCustomer = await this.customerRepository.insert(
        customerEntity,
      );

      const update = { customer: createCustomer.identifiers[0].id };

      await this.useryzcRepository.update(
        createUserYzc.identifiers[0].id,
        update,
      );

      return (
        this.useryzcRepository.findOneOrFail({
  ...
  async registerPsikolog(registerPsikologDto: RegisterPsikologDto) {
    try {
      ...
      psikologEntity.caseHandled = registerPsikologDto.caseHandled
      psikologEntity.spesialis = registerPsikologDto.spesialis

      const createPsikolog = await this.psikologRepository.insert(
        psikologEntity,
      );

      const update = { psikolog: createPsikolog.identifiers[0].id };
      ...
      return (
        ...
        this.psikologRepository.findOneOrFail({
          where: { id: createPsikolog.identifiers[0].id },
        })
      );
    } catch (error) {
      if (error instanceof EntityNotFoundError) {
  ...
  async login(loginDto: LoginDto) {
    try {
      ...
      const active = await this.useryzcRepository.findOne({
        where: { email: loginDto.email, status: StatusAcount.ACTIVE },
        relations: ['level_user', 'customer', 'psikolog'],
      });
      if (!active) {
        throw new HttpException(
  ...
  async changePassword(
    ...
    updateUserYzcDto: UpdateUserYzcDto,
  ) {
    try {
