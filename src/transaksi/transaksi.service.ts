...
export class TransaksiService {
    ...
    findAll(){
        return this.transactionRepository.findAndCount({relations: {customer: true,seminar: true,privateKonseling:true}})
    }
    ...
    async createTransaction(createTransactionDto: CreateTransactionDto){
        try {
            ...
            const findSeminar = await this.seminarService.findOne(createTransactionDto.seminar)
            ...
            transactionEntity.seminar = findSeminar
            ...
            detailEntity.seminar = transactionEntity.seminar
    ...
    async createTransactionKonseling(createTransactionKonseling: CreateTransactionKonselingDto){
      try {
            ...
            transactionEntity.customer = findCustomer
            transactionEntity.privateKonseling = element
            transactionEntity.bank = findBank
            ...
            detailEntity.transaction = insertTransaction.identifiers[0].id
            detailEntity.customer = transactionEntity.customer
            detailEntity.privateKonseling = transactionEntity.privateKonseling
            const insertDetail = await this.detailOrderRepository.insert(detailEntity)
    ...
    async findOne(id: string){
        try {
            return await this.transactionRepository.findOneOrFail({where:{id}, relations: {customer: true, seminar:true,privateKonseling:true}})
        } catch (error) {
